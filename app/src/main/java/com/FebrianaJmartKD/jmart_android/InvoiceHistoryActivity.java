...
/**
 * class Invoice History Activity
 *
...
import com.FebrianaJmartKD.jmart_android.model.Payment;
import com.FebrianaJmartKD.jmart_android.model.Product;

public class InvoiceHistoryActivity extends AppCompatActivity implements RecyclerViewInvoicesAdapter.ItemClickListener, RecyclerViewTransactionsAdapter.ItemClickListener {
    private static final Gson gson = new Gson();
    RecyclerViewInvoicesAdapter adapterInvoices;
    RecyclerViewTransactionsAdapter adapterTransactions;
    //TabSelector
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try{
            ...
            adapterTransactions = new RecyclerViewTransactionsAdapter(this, myTransactions, transactionProducts);
    ...
    public void fetchTransactions(List<Payment> myTransactions, int page, RequestQueue queue, boolean refreshAdapter){
        StringRequest fetchProductsRequest = new StringRequest(Request.Method.GET, "http://10.0.2.2:8086/payment/"+LoginActivity.getLoggedAccount().id+"/purchases/page?page="+page+"&pageSize=10", new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                ...
                try {
                    ...
                    Toast.makeText(getApplicationContext(), "Fetch transactions successful!", Toast.LENGTH_LONG).show();
                    if(refreshAdapter){
    ...
    public void fetchInvoices(List<Payment> storeInvoices, int page, RequestQueue queue, boolean refreshAdapter){
        StringRequest fetchProductsRequest = new StringRequest(Request.Method.GET, "http://10.0.2.2:8086/payment/"+LoginActivity.getLoggedAccount().id+"/page?page="+page+"&pageSize=10", new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                JsonReader reader = new JsonReader(new StringReader(response));
                try {
    ...
    public void fetchInvoiceProducts(List<Product> invoiceProducts, int page, RequestQueue queue, boolean refreshAdapter){
        StringRequest fetchInvoiceProductsRequest = new StringRequest(Request.Method.GET, "http://10.0.2.2:8086/product/"+LoginActivity.getLoggedAccount().id+"/page?page="+page+"&pageSize=10", new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                JsonReader reader = new JsonReader(new StringReader(response));
                try {
    ...
    public void fetchTransactionProducts(List<Product> transactionProducts, int page, RequestQueue queue2, boolean refreshAdapter){
        StringRequest fetchTransactionProductsRequest = new StringRequest(Request.Method.GET, "http://10.0.2.2:8086/product/"+LoginActivity.getLoggedAccount().id+"/purchases/page?page="+page+"&pageSize=10", new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                JsonReader reader = new JsonReader(new StringReader(response));
                try {
